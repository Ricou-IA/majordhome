...
- **Registre `src/lib/smsCampaigns.js`** (pur, testé `node --test scripts/sms-campaigns.test.mjs`) = source unique « campagnes envoyées par le code × variables » : `avis_j1` (`first_name`, `short_link`), `rappel_entretien` (`first_name`, `name`, `full_name`), `confirmation_rdv` (`prenom`, `date`, `heure`, `technicien`). L'onglet liste les variables, propose un texte suggéré (bouton) quand le gabarit est vide, et **bloque l'enregistrement sur variable inconnue** (`{{prénom}}` avec accent partirait tel quel chez le client : l'edge ne substitue que `[a-z0-9_]`).
...
- **Ajouter une campagne** = une entrée dans le registre + le gabarit saisi dans l'onglet. Un appelant doit traiter `campaign_template_missing` comme une information (toast), jamais comme une erreur bloquante.

### SMS confirmation de RDV (2026-09-12)
3ᵉ campagne, `confirmation_rdv`. Déclenchée par `ContractModal` après **toute pose réussie** d'un RDV d'entretien depuis la fiche contrat (CTA « Trouver le créneau optimisé » comme « Planifier manuellement »), dans `handleConfirmScheduling` → `savService.sendRdvConfirmation(...)` (non attendu : la pose est acquise, le SMS ne la retarde ni ne la fait échouer).
- Conditions : `settings.sms.enabled` (sinon silencieux), **un seul créneau** (plusieurs dates → toast info, pas de SMS), mobile FR (`isMobileFR`, sinon toast info). Variables : `prenom` = `contract.client_first_name`, `date` = « mardi 13 octobre » (`formatSmsDate`), `heure` = « 9h30 » / « 14h » (`formatSmsHour`), `technicien` = prénom du membre (`smsNameForMember`, repli nom complet ; plusieurs techniciens → « X et Y »).
- Issues : envoyé → toast succès ; `campaign_template_missing` → toast info vers Settings → Organisation → SMS ; autre échec → toast warning + `logger.error`. Trace `sms_logs` avec `client_id` + `intervention_id` (carte entretien).
- **Hors périmètre** : les poses depuis le kanban entretien et l'onglet Tournées (`useJourneePose`) n'envoient pas de SMS.
