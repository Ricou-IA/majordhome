...
- **Envoi** : `savService.sendEntretienReminder({contractId, clientId, clientFirstName, clientName, clientPhone, orgId})` → edge `sms-send` (campagne `rappel_entretien`, depuis le 2026-08-11 — l'ancien webhook N8N `VITE_N8N_WEBHOOK_SMS_RAPPEL` n'est plus appelé). Mono-mobile, validation `isMobileFR` (`src/lib/phoneUtils.js`, partagé avec `sendAvisRequest`).
...
- **Composition** : le gabarit (SMS seul chez Mayer, `deburr: true` → GSM-7) vit dans `settings.sms.templates.rappel_entretien`, éditable dans Settings → Organisation → SMS (voir ci-dessous) ; `full_name` = nom+prénom MAJUSCULES construit côté service. Log `sms_logs` posé par l'edge (`campaign_name='rappel_entretien'`). Pas de writeback intervention (contrairement à `avis_j1` qui pose `sms_avis_sent`). *(Historique : workflow N8N `mayer-sms-rappel-entretien`, remplacé.)*
...
### Gabarits SMS / WhatsApp — Settings → Organisation → SMS (2026-09-12)
Onglet `SmsTab.jsx` (org_admin), 4ᵉ onglet de `/settings/organization`. Édite `core.organizations.settings.sms.templates` via `useOrgSettings()` ; l'**identité d'expéditeur** (`enabled`, `sms_from`, `whatsapp_from`, `short_link_base`) y est affichée en **lecture seule** — posée par la plateforme à l'onboarding (nom d'expéditeur SMS à déclarer auprès de l'opérateur), comme la clé Twilio.
- **Registre `src/lib/smsCampaigns.js`** (pur, testé `node --test scripts/sms-campaigns.test.mjs`) = source unique « campagnes envoyées par le code × variables » : `avis_j1` (`first_name`, `short_link`), `rappel_entretien` (`first_name`, `name`, `full_name`), `rappel_rdv` (`prenom`, `date`, `heure`, `technicien`). L'onglet liste les variables, propose un texte suggéré (bouton) quand le gabarit est vide, et **bloque l'enregistrement sur variable inconnue** (`{{prénom}}` avec accent partirait tel quel chez le client : l'edge ne substitue que `[a-z0-9_]`).
- **« Coût de ce message : N SMS »** sous chaque gabarit SMS, calculé sur un **exemple rendu** (`sampleVarsFor` : prénom, date longue « mercredi 16 septembre », lien court sur le domaine de l'org) — jamais sur le gabarit brut (les `{{ }}` comptent double en GSM-7 et les variables changent la longueur ; le texte suggéré affichait « 2 » pour un vrai coût de 1). Règle : 160 caractères par SMS (153 en concaténé) en alphabet GSM-7, **70** dès un caractère hors alphabet (`ê â î ô û ë`, apostrophe typographique `’`, guillemets). Case « Retirer les accents à l'envoi » (= `deburr`, appliqué au SMS **et** au WhatsApp). L'aperçu affiché EST le rendu de l'edge : `renderSmsTemplate` / `deburrSms` vivent dans le registre et `_shared/sms.ts` les importe (copie synchronisée) — plus de copie du rendu côté Deno.
- Sauvegarde = objet `sms` **complet** (merge JSONB niveau 1), gabarits vides retirés (→ `campaign_template_missing` à l'envoi), clés inconnues du code préservées et signalées. Sans `whatsapp_from`, un gabarit WhatsApp est conservé mais ignoré : seul le SMS part.
- L'onglet porte aussi le **réglage du rappel automatique** `settings.sms.rappel_rdv = { mode: off|veille|hebdo, jour: 1-7, heure: 7-20 }` (défaut `off`) — voir section suivante.
- **Ajouter une campagne** = une entrée dans le registre + le gabarit saisi dans l'onglet. Un appelant doit traiter `campaign_template_missing` comme une information (toast / compteur), jamais comme une erreur bloquante.
- Le registre et `phoneUtils.js` sont **copiés vers `supabase/functions/_shared/`** par `npm run sync:tournee-engine` (test d'égalité `scripts/tournee/sync-engine.test.mjs`) : l'edge formate date / heure / technicien avec exactement le code du front. Ne jamais éditer les copies.

### Rappel automatique des RDV d'entretien — cron `sms-rappel-rdv` (2026-09-12)
Le planning sert de pré-planification (l'humain valide par téléphone) : **pas de SMS à la pose**. À la place, un cron rappelle au client son entretien, campagne `rappel_rdv`, selon un **réglage par org** éditable dans Settings → Organisation → SMS :
- `veille` : chaque jour à `heure`, les RDV du lendemain · `hebdo` : le `jour` choisi à `heure`, les RDV des 7 jours suivants · `off` (défaut) : rien. Changer de « lundi » à « la veille » = un clic, sans redéploiement.
- **Mécanique** : pg_cron `sms-rappel-rdv` **horaire** (`0 5-19 * * *` UTC = 7h-20h Paris) → edge `sms-rappel-rdv` (`verify_jwt:false`, `requireSharedSecret(MDH_CRON_SECRET)`) → pour chaque org `settings.sms.enabled` : `planifierRappelRdv(config, maintenantParis)` décide s'il y a une fenêtre **maintenant** (heure H ou H+1 : la 2ᵉ passe retente les échecs) → RPC `sms_rappel_rdv_candidates(org_core, from, to)` (RDV `maintenance` + `scheduled`, `client_notified_at IS NULL`, téléphone/prénom de la fiche client sinon photo du RDV, techniciens en jsonb, fenêtre ≤ 31 j) → `isMobileFR` → `sendCampaignSms` (`_shared/sms.ts`, même cœur que `sms-send`) → RPC `sms_rappel_rdv_mark_notified`.
- **Anti-doublon = `appointments.client_notified_at`** (colonne préexistante, jamais utilisée avant) : posée après un envoi réussi, remise à NULL par le trigger `reset_client_notified_on_reschedule` (`BEFORE UPDATE OF scheduled_date, scheduled_start`) → un RDV déplacé est rappelé de nouveau, jamais deux fois pour le même créneau.
- **Appels manuels** (secret partagé, ex. via `net.http_post` depuis SQL) : body `{ dry_run: true }` liste ce qui partirait sans envoyer ni marquer (numéros masqués) ; `{ org_id }` limite à une org ; `{ force: true }` ignore jour/heure ; `{ from, to }` fenêtre explicite. Réponse : rapport par org (`candidates`, `sent`, `no_mobile`, `failed`, `mark_failed`, `skipped`).
- **Limites assumées** : un RDV créé après la passe de la fenêtre n'est rappelé qu'à la passe suivante (veille : le lendemain ; hebdo : jamais s'il tombe dans la semaine en cours — c'est l'appel humain qui couvre) ; un changement de technicien seul ne redéclenche pas ; SAV (`service`) et installations exclus ; fuseau Europe/Paris pour toutes les orgs (`majordhome.organizations.timezone` non lu).
- `sms-send` a été refactorée le même jour : le cœur (rendu `{{}}`, lien court, Twilio WhatsApp→SMS, pré-log/clôture `sms_logs`) vit dans `_shared/sms.ts` ; l'edge ne garde que validation + membership + mise en forme HTTP (réponses inchangées).
