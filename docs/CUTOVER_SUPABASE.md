...
1. **~~Trois~~ UN workflow N8N écrit encore dans l'ANCIENNE base** — reste `Mayer Entretien Web - Souscription Contrat`. Il passe par des nœuds Postgres et un identifiant partagé (« Postgres account »), commun avec Baikal et Pack Vendeur — d'où le choix de ne pas le modifier. **Décision : le convertir en edge function plutôt que repointer** (cf. [[project-sortie-n8n-vers-edge-functions]]). Dormant : le formulaire web est jugé sans trafic en août.
