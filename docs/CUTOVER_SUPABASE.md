...
0. 🔴 **LE SITE VITRINE ÉCRIVAIT ENCORE DANS L'ANCIEN PROJET — corrigé le 2026-08-12.** Le plus gros trou de cette bascule, et il n'était pas dans la liste : `mayer-energie.fr` (dépôt `C:\Dev\Landing Page - Mayer`, Next.js) appelle Supabase depuis ses routes `app/api/*` avec `NEXT_PUBLIC_SUPABASE_URL` + `SUPABASE_SERVICE_ROLE_KEY`, et ces variables pointaient toujours vers `odspcxgafcqxjzrarsqf`. **Treize appels RPC dans onze routes** — entretien, bornes de recharge, aides, pellets, devis PAC, Tesla, contact — écrivaient donc dans une base que le CRM ne lit plus. Aucune perte constatée : le seul enregistrement web postérieur à la bascule (10/08 15:14) précédait le dump, et août n'a apporté aucun trafic ensuite. **C'est de la chance, pas de la conception.**

   Corrigé : les 3 variables repointées sur Vercel, puis vérifié **de bout en bout** par une vraie soumission — contrat créé dans le nouveau projet, ancien projet vide, données de test purgées. Contrôle préalable indispensable avant de rebasculer un site : les 6 RPC qu'il appelle (`create_website_lead`, `create_aide_lead`, `create_aide_request`, `inscrire_prospect_pellets`, `process_web_entretien`, `verify_quote`) existent avec `service_role` des deux côtés — sinon un formulaire casse en silence.

   **Leçon** : le périmètre d'une bascule, ce n'est pas « l'app + les automatisations ». C'est **tout ce qui détient une URL de projet**, y compris un dépôt séparé qu'on n'a pas ouvert.

1. ✅ **~~Trois~~ ZÉRO workflow N8N n'écrit dans l'ANCIENNE base.** `Mayer Entretien Web - Souscription Contrat` n'a pas eu besoin d'être converti : en l'ouvrant, on a découvert que la route `/api/entretien` du site **faisait déjà tout** — l'écriture via `process_web_entretien` ET l'email par Resend. L'appel à N8N qui suivait refaisait la **même** écriture et envoyait un **second** email Gmail. Chaque souscription créait donc le contrat deux fois et le client recevait deux emails. Appel supprimé (site, `main`, `e2317e5`), vérifié : un seul email reçu. Le workflow devient du code mort — **à désactiver**, ce qui ferme au passage l'injection SQL de son nœud « Préparer données » (champs numériques concaténés sans coercition depuis un webhook public ; correctif `num()` appliqué le 2026-08-12 en attendant).

   ✅ **`SMS Avis Client` + `SMS RDV` convertis le 2026-08-11** en une seule edge `sms-send` (commit `06bae48`). Motif du choix : un workflow ne sait pas être multi-client (expéditeur, texte, domaine, clés en dur) — or le stack technique est destiné à être revendu. Twilio est mutualisé côté plateforme comme Resend ; ce qui est propre au client est l'**identité d'expéditeur**, rangée dans `settings.sms` (numéro, nom d'expéditeur, domaine du lien court, gabarits + option `deburr`). Vérifié par 2 envois réels : texte identique au caractère près, trace complète dans `sms_logs`. Les 2 workflows restent **actifs mais non appelés** — filet de retour arrière tant qu'un envoi réel depuis l'app n'a pas eu lieu ; à désactiver ensuite, avec les `VITE_N8N_WEBHOOK_SMS_*` chez Vercel.

   **Audit N8N complet (2026-08-11)** : les 12 workflows ont été passés en revue. `Contact Lead`, `Urgence SAV` et `Slack Interactions` ne touchent **aucune base** (webhook → Slack → Gmail) — la liste de 3 était donc exhaustive. ⚠️ Mine repérée au passage : `Slack Interactions` construit un objet rendez-vous puis **ne l'écrit nulle part**, tout en affichant « ✅ RDV PLANIFIÉ » dans Slack. Inactif aujourd'hui (le bouton qui l'ouvre n'existe que dans la version **non publiée** d'`Urgence SAV`) — **ne pas publier ce brouillon en l'état**, il produirait des confirmations fantômes.
