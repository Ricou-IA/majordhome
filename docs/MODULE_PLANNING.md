...
- **Bloc B stage 5** : nettoyage frontend FAIT (2026-08-26) — `chantierSlots.service.js`, `chantierSlotKeys`, `useInterventionSlots`, `getChantierInterventionByLeadId`, mutations `createChantierIntervention/createSlot/deleteSlot` et méthodes slots du service supprimés. **DB FAIT aussi (2026-08-26)** : DROP vue `majordhome_intervention_slots` (migration `20260826_1`, 5 slots historiques non migrés — décision Eric : repartir propre ; les lignes `majordhome.interventions` restent). Stage 5 clos.
