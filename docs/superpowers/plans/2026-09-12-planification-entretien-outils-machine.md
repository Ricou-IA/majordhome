...
- SMS de confirmation après la pose : **hors tranche 1** (aucun gabarit ni onglet Settings SMS n'existe ; règle « pas de config sans UI »). Signalé en tâche de suite — ✅ livré le 2026-09-12 en tâche de suite (onglet Settings → Organisation → SMS + campagne `confirmation_rdv`, cf. `docs/MODULE_MAILING.md`).
...
- [ ] **Step 5 : tâches de suite (spawn_task)** — (a) ✅ SMS de confirmation après pose (gabarit `confirmation_rdv` + onglet Settings SMS — livré 2026-09-12) ; (b) `settings.tournees` éditable dans `/settings` (dette connue, + `horizon_ouverture_jours`) ; (c) renommage profil Hermes `majordhome` → `mayer` ; (d) tranche 2 : RPC `entretien_book_slot` + `identify_client`.
