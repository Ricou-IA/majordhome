...
- Gotchas SMS : l'edge ne substitue que `{{[a-z0-9_]+}}` → `{{prénom}}` partirait tel quel (l'onglet bloque l'enregistrement) ; `deburr` s'applique au SMS **et** au WhatsApp ; sans `whatsapp_from` le gabarit WhatsApp est ignoré ; `’ ê â î ô û « »` font basculer un SMS en UCS-2 (70 car./segment). `deburrSms` du registre est une **copie** du `deburr()` de `_shared/sms.ts` : toute évolution touche les deux.
---
