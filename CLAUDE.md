...
> **Référence par domaine** : `docs/DATABASE.md`, `docs/COMPONENTS.md`, `docs/SPRINT_LOG.md`, et `docs/MODULE_*.md` (Mailing, Pennylane, Planning, Entretiens, Tournées). Historique des modifs : git.
...
- **RPC `public.update_majordhome_lead(p_lead_id uuid, p_updates jsonb)`** : RPC SECURITY DEFINER générique pour patcher partiellement un lead côté frontend. Préférable à `supabase.schema('majordhome').from('leads').update(...)` qui renvoie 406 (schema non exposé). Pattern : `supabase.rpc('update_majordhome_lead', { p_lead_id, p_updates: { client_id, updated_at: new Date().toISOString() } })`. Utilisée dans ~15 endroits (leads.service, geocoding.service, hook ensureClientForLeadFromPennylane, etc.). Le filtre `org_id` est appliqué côté RPC (check membership). **⚠️ Liste de colonnes EXPLICITE** : une nouvelle colonne de `leads` est ignorée en silence par la RPC tant qu'on n'y ajoute pas sa ligne `CASE WHEN p_updates ? '…'` (vécu avec `planned_team_size` / `planned_days`, 2026-09-22). Le `REVOKE FROM PUBLIC, anon` manquait jusqu'au 2026-09-22.
...
- **Un fichier `sql/*.sql` ou une doc « ✅ FAIT » ne prouve pas qu'un objet existe en base.** Avant de déboguer un module qui « échoue en silence », vérifier d'abord l'existence de ses objets en prod (`to_regclass('majordhome.<table>')`, `to_regclass('public.majordhome_<vue>')`) et l'inscription de sa migration dans `supabase_migrations.schema_migrations`. Vécu 2026-09-20 : module Prospection livré en mars 2026 sans qu'aucune de ses tables n'ait jamais existé (migration `sql/migration_prospects.sql` « à exécuter dans le SQL Editor », jamais jouée) — six mois de PGRST205 avalés par les hooks. Toute nouvelle table part d'une migration versionnée `supabase/migrations/` répétée sur le harnais `scripts/migration-rehearsal/`, jamais d'un fichier « à exécuter à la main ».
- **Harnais de répétition = sous-ensemble déclaré** : `snapshot.mjs` liste TABLES / VIEWS / FUNCTIONS / TRIGGER_TABLES photographiés. Une migration qui touche une vue ou une fonction absente échoue en « relation does not exist » ; un trigger posé en prod dont la fonction n'est pas listée casse le chargement du schéma (vécu 2026-09-22, `equipment_unlinked_purge_children`). Étendre la liste, ne pas contourner.
...
- `majordhome_prospects` → miroir simple **auto-updatable** de `majordhome.prospects` (lecture ET écriture front : upsert `ON CONFLICT (org_id, module, siren)` du screener, update/delete du drawer — migration `20260920_2`). Pas de colonne calculée
- `majordhome_prospect_interactions` → miroir + `created_by_name` (sous-requête scalaire sur `profiles`, reste insérable). Timeline append-only : SELECT/INSERT seulement
...
- **Aucun trigger `auth.users` ne crée `core.profiles`** : toute voie de création d'utilisateur (`create-user`, SSO, import) doit INSÉRER le profil elle-même via `.schema('core')` (service_role) ; le trigger `core.sync_profile_org_membership` pose alors l'adhésion (rôle = `app_role` recopié tel quel → réaligner sur `org_admin` / `team_leader` / `member` derrière). Les vues publiques `profiles` / `organization_members` ne sont pas des cibles d'écriture serveur (`organization_members` est un JOIN non insérable). Vécu 2026-09-21 : `create-user` répondait « succès » sans rien créer (UPDATE sur la vue `profiles` = 0 ligne, INSERT sur `organization_members` refusé, `{ error }` jamais lu) → deux comptes auth orphelins ; réécrite `127a553` (email déjà inscrit ⇒ compte réutilisé et complété).
- **Une edge function qui écrit doit lire `{ error }` de CHAQUE écriture** et répondre 5xx explicite — une réponse 201 après un INSERT ignoré est l'échec silencieux type.
...
- **Auto-reload sur `vite:preloadError`** (`main.jsx`) : si un chunk hashé de l'ancienne build a disparu post-déploiement (Vercel renvoie `index.html` → erreur MIME sur le module dynamique), Vite émet `vite:preloadError` → on recharge la page UNE fois. Garde anti-boucle via `sessionStorage` clé `mdh:chunk-reload-at` (fenêtre 10s) — récupération auto post-deploy sans intervention.
...
- **SMS / WhatsApp (2026-09-12)** : campagnes = registre pur `src/lib/smsCampaigns.js` (`avis_j1`, `rappel_entretien`, `rappel_rdv`, copié vers `_shared/` par `npm run sync:tournee-engine`, ne jamais éditer la copie) ; gabarits dans `settings.sms.templates`, **éditables Settings → Communication → SMS** (`SmsTab`). Ajouter une campagne = registre + gabarit saisi dans l'onglet, jamais un texte en dur ni un `UPDATE` SQL. **Un seul cœur d'envoi : `_shared/sms.ts::sendCampaignSms`** (rendu, lien court, Twilio WhatsApp→SMS, pré-log/clôture `sms_logs`), utilisé par `sms-send` (membership) et `sms-rappel-rdv` (cron, secret) — jamais un 2ᵉ appel Twilio ailleurs.
- **`campaign_template_missing` est une information, pas une erreur** (`toast.info` côté front, compteur `skipped` côté cron) : l'action métier qui déclenche l'envoi n'échoue jamais. `settings.sms.enabled !== true` → silencieux.
- **Rappel des RDV = réglage d'org** (`settings.sms.rappel_rdv = { mode: off|veille|hebdo, jour, heure }`, défaut `off`), cron `sms-rappel-rdv` **horaire** ; anti-doublon = `appointments.client_notified_at`, remis à NULL par trigger si date/heure changent. Périmètre `maintenance` + `scheduled`. Gotchas : seul `{{[a-z0-9_]+}}` est substitué (`{{prénom}}` partirait tel quel) ; `’ ê â î ô û « »` basculent en UCS-2 (70 car./segment) → `deburrSms` ; coût affiché en **SMS** sur un exemple rendu.
...
- **Déplanification** = `savService.unscheduleEntretien` (miroir de `scheduleEntretien` : supprime les RDV liés via `deleteAppointment`, donc Google Calendar compris) + trigger DB `trg_intervention_unschedule` (Planifié → À planifier ⇒ `scheduled_date` NULL, enfants certificat vierges purgés — côté DB car la policy DELETE d'`interventions` = org_admin seul, un DELETE front serait un no-op silencieux). Ne jamais remettre une carte en À planifier par `updateWorkflowStatus` seul (2026-09-18 : tournée déplanifiée, 3 RDV fantômes).
- Gotcha backfill : un RDV passé ≠ entretien non fait (scope backfill = RDV À VENIR uniquement).
- **Couleur d'un RDV = couleur de la PERSONNE, pas du type** : source unique `majordhome.team_members.calendar_color` (éditable Settings → Équipe, unicité contrôlée). Résolution dans le module PUR `src/lib/planningEvents.js` (testé `scripts/planning-events.test.mjs`) : `resolveAppointmentColor` prend la couleur du propriétaire (commercial préféré pour VT/agence, sinon technicien), override **violet `#6D28D9` = statut « facturé »** — couleur RÉSERVÉE, ne jamais l'assigner à quelqu'un — et fallback slate `#94A3B8`. Une personne présente en technicien ET en commercial (Philippe, Michel) est unifiée par `profile_key` (= `team_members.user_id` = `commercials.profile_id`) : sans ça elle apparaît deux fois dans les filtres et ses RDV changent de couleur selon le type.
- **`appointments.assigned_commercial_id` (sans FK) porte deux référentiels** : `commercials.id` (VT posées depuis le pipeline) ou `team_members.id` (VT posées depuis le planning) — 203 / 25 mesurés en prod le 2026-09-17. Tout consommateur doit résoudre les deux, joints par le profil (`commercials.profile_id` = `team_members.user_id`) : cf. `resolveProfileIds` (edge `google-calendar-sync`) et la résolution couleur / édition de `src/lib/planningEvents.js` côté front. Ne chercher qu'une table = commercial ignoré en silence.
- **⚠️ Affichage calendrier ≠ « 1 RDV = 1 carte »** : un RDV à ≥2 techniciens est éclaté en 1 bloc par technicien (chacun sa couleur) via `expandAppointmentBlocks`. FullCalendar exige un id unique par event → l'id du bloc devient `${appointment.id}__${techId}`. **Toujours lire l'id réel via `event.extendedProps.id`** dans les handlers drag/resize/clic, jamais `event.id`. Facturé → 1 bloc violet ; mono-tech ou RDV commercial → 1 bloc ; blocs restreints aux techniciens visibles si un filtre équipe est actif.
- **Filtres planning** : `{ kinds: {intervention, commercial}, memberProfileKeys[], appointmentType, status }`, buckets `COMMERCIAL_TYPES` / `TECHNICIAN_TYPES` de `planningEvents.js` (ne pas recopier les listes ici, elles bougent). Tout type hors buckets tombe dans `other` et **reste toujours visible** : ne jamais masquer un RDV au prétexte que son type est inconnu du filtre.
- **1 membre d'org actif = 1 ressource planning `majordhome.team_members`** (colonne calendrier, assignation RDV, couleur). `create-user` ne la crée PAS (elle ne pose que `auth.users` + `core.profiles` + `core.organization_members`) : elle est garantie par la RPC idempotente `team_member_ensure_for_user(p_core_org_id, p_user_id, p_color)` (SECURITY DEFINER, org_admin only, relie une ressource orpheline de même email avant d'insérer), auto-appelée depuis `/settings/team` pour tout membre qui n'en a pas. **Toute nouvelle voie de création de membre** (SSO, import, edge function) doit l'appeler — sinon le membre n'apparaît dans aucune assignation de RDV et sa couleur est inéditable : échec silencieux (vécu sur 2 membres Mayer). Idempotence structurelle : index unique partiel `(org_id, user_id) WHERE user_id IS NOT NULL`.
...
- **SMS de rappel d'entretien** (bulle sur l'onglet Programmation, contrats « à planifier » uniquement, permission `entretiens.create`) : l'état « déjà relancé cette année » n'a **pas de colonne dédiée** — il est dérivé de `majordhome_sms_logs` (`campaign_name='rappel_entretien'`, `sent_at ≥ 01/01`, cache `smsKeys.remindedClients(orgId, year)`), donc remis à zéro tout seul au 1ᵉʳ janvier. ⚠️ **Cette liste EST le seul garde-fou anti-doublon** : (a) ne jamais avaler son erreur — un Set vide signifie « personne n'a été relancé » et rouvre l'envoi à TOUS en silence ; (b) l'état se dérive **à chaque rendu**, jamais figé dans un `useState` (la ligne se monte avant la réponse de la requête) ; (c) liste inconnue (`undefined` : chargement ou échec) → bulle **verrouillée**, on ne devine pas. Limite V1 assumée : `sms_logs` ne porte pas de `contract_id`, l'état est indexé par client → un client multi-contrats est marqué relancé après un seul SMS.
## Module Tournées (moteur d'entretien · souplesse · journée pleine) → `docs/MODULE_TOURNEES.md`
Règles qui mordent :
- **Moteur = `src/lib/tournee/*`, modules PURS, copiés pour Deno** (`npm run sync:tournee-engine` → `supabase/functions/_shared/tournee/`, test de synchro dans `audit:quality`). Ne jamais éditer les copies ; après toute modif : sync + redéployer `slots-propose` **et** `tournees-figer`. Deno type-check lit les JSDoc des `.js` : une signature exportée non documentée casse `deno check`.
- **Réglages = `settings.tournees`**, source unique `reglages.js`, éditables Settings → Organisation → Tournées (règle « pas de config sans UI ») ; l'onglet renvoie l'objet complet (merge JSONB niveau 1). **Budget** = `team_members.daily_work_minutes` (Settings → Équipe) = temps d'homme **trajets compris**, plafond + `depassement_journee_minutes` (30).
- **Temps de travail du moteur = barème × gain multi-équipements (10 % dès 2 lignes), le bloc n'est qu'un dessin** : `chargerJournees` remplace `duration_minutes` d'un Entretien à contrat (valeur saisie dans `duration_minutes_saisie`) ; le bloc suit au figeage. Barème faux ⇒ corriger le barème (Tarification), jamais le bloc. **Pose à la main = bloc contrat** (`fixedDuration` : un clic pose la durée entière).
- **Souplesse OPT-IN** : `toleranceDe(rdv, { souplesse: true, … })` — sans ce flag, tolérance ponctuelle quel que soit `time_flex_minutes`. Seul un appelant qui sait **écrire** les décalages la demande (`proposerPourContrat` → `scheduleEntretien({ decalages })`, consolidation) ; le remplissage de journée refuse tout placement qui supposerait un voisin déplacé. `time_flex_minutes` NULL = défaut d'org ; `hour_confirmed_at` ⇒ figé ; ancre = `announced_start` (posée à la création, ré-ancrée au drag — un drag ne fige pas, figer est un geste). Seuls `maintenance` et `service` sont adaptables.
- **Le voisin se glisse AVANT la pose**, relu en base, refusé (`decalage_refuse` + raison) si son état a changé, remis en place si la pose échoue ; une carte créée pour l'occasion redescend en « À planifier » si la pose échoue (carte fantôme vécue le 2026-09-12).
- **Une journée pleine se fige toute seule** (reste utile < `reste_utile_min_minutes`, à partir de demain) — edge `tournees-figer` (cron horaire, `MDH_CRON_SECRET`) → RPC `tournees_figer_journee` (service_role only, tout ou rien) → heures au 5 min supérieur, 🔒. **Pas de figeage la veille** (Eric : « si c'est plein depuis 10 jours, pourquoi attendre ? »). **SMS `heure_de_passage` = réglage `figer_sms`, OFF pour l'instant** (cron et bouton). Pleine mais impossible → **tableau de bord de l'admin** (`JourneesAArbitrer`), pas l'onglet Tournées.
- **La journée commence au dépôt à l'ouverture** : le trajet vers le premier client compte ; départ anticipé réservé à un RDV **figé** à l'ouverture. Un figé atteint « en retard » selon nos estimations est un **fait** (`figesSontDesFaits`), pas un blocage. Quand rien ne tient, lire `sequencerTournee().diagnostic` (journée telle que posée), jamais `raison` seule (dernière permutation, arbitraire).
- **Contrat → créneaux** (CTA fiche contrat, edge `slots-propose`, `requireOrgMembership`) : classement par score (coût + temps perdu), **présentation chronologique**, trajet / travail / reste utile à l'écran, trajet max entre deux clients (45 min, dépôt exempté) → « ouvrir une nouvelle journée » ; Mapbox via `MDH_MAPBOX_TOKEN`, repli vol d'oiseau **toujours signalé** (`estime: true`). Compétences = `team_member_skills` (type × rôle, **coché = compétent**, cf. Module Référentiel équipements — `specialties` « vide = polyvalent » est abandonné). Adresse BAN à la saisie (`client_set_location`), précision commune suffisante.
...
- **Remise exceptionnelle** (`contracts.exceptional_discount`, migration `20260921_1`) : appliquée APRÈS la dégressivité — total = sous-total − dégressivité − remise exceptionnelle, via `calculateContractTotal(items, discounts, exceptionalDiscount)` du module PUR `src/lib/contractPricing.js` (sorti de `pricing.service.js`, ré-exporté pour ContractSign / ContractPdfSection / ContractPricingSection). **Toute surface qui chiffre un contrat passe par `computeContractLines` de ce module**, jamais par `contract_pricing_items` (vides sur la plupart des contrats). Reprise identique PDF / signature / facture d'entretien.

## Paramétrage par module de l'offre (Settings, 2026-09-13)

- **Découpage de l'offre = `src/lib/modules.js`** (module PUR, testé `node --test scripts/modules.test.mjs`, inclus dans `audit:quality`) : `socle` (Organisation, Équipe, Droits d'accès, Équipements, Fournisseurs & catalogue), `entretiens` (Tarification, Tournées), `communication` (Emails, SMS & WhatsApp), `solaire`, `thermique`. **Socle = ce qui décrit l'entreprise et son parc client ; un module = ce qui le valorise.** Commercial (pipeline, devis, webshop) et chantiers sont dans le socle (décision Eric). Toute nouvelle page de paramétrage se déclare dans le registre (le test refuse une tuile sans route dans `routes.jsx`) et utilise le gabarit `pages/settings/SettingsPage.jsx`. La sidebar et l'activation par org (`settings.modules`) se brancheront sur ce même registre — ne pas recopier la liste ailleurs.
- **Référentiel ≠ tarification** : `pricing_equipment_types` porte deux natures. Code / libellé / catégorie / actif = référentiel du socle, édités dans Settings → Équipements (`equipements/TypesTab.jsx`). Durée d'entretien, tarif unitaire, mois déconseillés = module Entretiens, édités dans Settings → Tarification → « Durées d'entretien » (`pricing/DureesTab.jsx`), mise à jour PARTIELLE (`pricingService.updateEquipmentType` n'écrase que ce qu'on envoie). Un type se crée d'abord (socle), se valorise ensuite (module).
- Le champ « Ordre » (`sort_order`) n'est plus saisi nulle part : un nouvel élément se range en fin (`pricing/ordre.js::prochainOrdre`), l'ordre existant est conservé à l'édition.
- **Toute icône citée dans `src/lib/modules.js` doit figurer dans `ICONS` de `pages/Settings.jsx`**, sinon la tuile affiche un « ? » (`HelpCircle`) sans aucune erreur — vécu sur la tuile Facturation Pennylane le 2026-09-21. À ajouter au test `modules.test.mjs` si ça se reproduit.

## Module Référentiel équipements (Settings → /settings/equipements, 2026-09-12)

Remplace l'enum Postgres `majordhome.equipment_category` (vocabulaire Mayer figé) par un référentiel à 2 niveaux **par org**. Spec `docs/superpowers/specs/2026-09-12-referentiel-equipements-tarifs-competences-design.md`. **M1 (expansion `20260913_1`) en prod ; M2 (contraction `20260920_1`, drop de l'enum, irréversible) après observation ≥ 1 semaine** — tant que M2 n'est pas passée, `equipments.category` existe encore mais ne doit plus être écrite.

- **2 niveaux** : `majordhome.equipment_categories` (code immuable `^[a-z0-9_]+$`, libellé, ordre, gabarit de certificat `certificate_profile` ∈ liste fermée niveau app, TVA par défaut) → `majordhome.pricing_equipment_types` (`category_id NOT NULL`, FK composite même org ; nom physique conservé). **Plus aucune constante de libellés dans le front** : tout passe par `useEquipmentReferential()` + `src/lib/equipmentReferential.js` (index, `grouperTypesParCategorie`, `libelleEquipement`, `profilParCode`).
...
- **Gotcha migration** : un trigger dont la liste `UPDATE OF` cite une colonne dépend de cette colonne — la supprimer exige de DROP le trigger d'abord. Toute migration structurelle se répète sur `scripts/migration-rehearsal/` (snapshot prod + cluster PostgreSQL local) avant livraison.
- Reliquats signalés : 353 équipements sans type ; icônes par type (`equipmentIcons.js` code en dur les codes Mayer) ; libellés côté portail client (Sprint 8).

## Module Tarification (Settings → /settings/pricing)

Valorisation per-org (P0.0.6 pricing per-org, 2026-05-21). Accès `org_admin` only. Depuis le 2026-09-13, les types d'équipement (référentiel) se gèrent dans Settings → Équipements ; cette page ne garde que la valorisation.

- **Page** : `src/apps/artisan/pages/settings/PricingSettings.jsx` — onglets : Zones / Tarifs (matrice zone × type) / Remises volume / Options / Durées d'entretien (`pricing/DureesTab.jsx`)
- **Hook admin** : `usePricingAdmin()` dans `src/shared/hooks/usePricing.js` — expose `{zones, equipmentTypes, rates, discounts, extras}` (incluant inactifs) + mutations CRUD scopées automatiquement sur `useAuth().organization.id`
- **Hook prod** : `usePricingData()` (existant) — filtre `is_active=true` pour les formulaires contrat
- **Service** : `pricing.service.js` — lectures via vues publiques `majordhome_pricing_*` (RLS via `security_invoker`), écritures via **vues publiques writable** : zones/types/remises/options sur leurs vues `majordhome_pricing_*` (miroirs updatable), tarifs sur `majordhome_pricing_rates_write` (la vue `_rates` est JOINée donc non-updatable). **Ne JAMAIS écrire via `.schema('majordhome')`** (schema non exposé par PostgREST → `PGRST106 Invalid schema: majordhome`, bug grille tarifaire corrigé 2026-06-23 : tout le CRUD écrivait en `.schema('majordhome')`, silencieusement cassé depuis le durcissement Sem 0)
- **Tables `majordhome.pricing_*`** : `pricing_zones`, `pricing_equipment_types`, `pricing_rates`, `pricing_discounts`, `pricing_extras` — toutes avec `org_id NOT NULL` + FK `core.organizations` + UNIQUE composites `(org_id, …)`. **RLS (durcissement 2026-06-23)** : `SELECT` = niveau membre (`org_id IN (org_members)`, les formulaires contrat lisent les tarifs) ; `INSERT/UPDATE/DELETE` = **`org_admin` only** (`… AND om.role='org_admin'`)
...
Configuration multi-tenant de l'identité de l'org (livraison Task 1-15, 2026-05-22). Accès `org_admin` only — RouteGuard `resource="settings"` + guard `isOrgAdmin` in-component (`<Navigate to="/settings" replace />` si non-admin). Depuis le 2026-09-13 (paramétrage par module), Organisation n'a **que 3 onglets** : Tournées → `/settings/tournees` (`entretiens/TourneesTab.jsx`), SMS → `/settings/sms` (`communication/SmsTab.jsx`), `from_email` / `reply_to` / domaine Resend → Settings → Communication → Emails (`communication/EmailsTab.jsx`) — mêmes clés `settings.*`, chaque écran n'envoie que les siennes (merge JSONB niveau 1).
...
- **Page** : `src/apps/artisan/pages/settings/OrganizationSettings.jsx` — sidebar gauche + 3 onglets :
  ...
  - **Coordonnées** (`organization/ContactTab.jsx`) : `address`, `postal_code`, `city`, `phone` (auto-format FR), `website_url` (auto-prefix https au save). `domain` calculé auto depuis `from_email` (édité dans Communication → Emails), `portal_url` = constante app (singleton)
...
- **PL a le dernier mot sur un refus** : `invariant_winning` ne force `accepted` que sur un statut indécis (`null`/`pending`/`draft`/`expired`) ; un refus explicite passe et retire `is_winning_quote`. Avant, le trigger réécrivait le refus AVANT écriture, la valeur stockée ne bougeait jamais et le cron comptait un update réussi — échec silencieux permanent. ⚠️ Dans le cas indécis le statut stocké diverge VOLONTAIREMENT de PL et ne se réaligne jamais seul (no-op assumé du cron toutes les 15 min).
...
- **Un flux serveur ne crée JAMAIS de lead** (Eric, 2026-09-16, après les doublons DURAND / GOTTARDI fabriqués par la machine). `upsert_pennylane_lead` (cron `pennylane-sync-cron` horaire + job `pennylane-backfill-quotes`) ne rapproche que des leads **ACTIFS** (email, téléphone, `client_id`) et renvoie `no_lead` sinon : le devis reste « Non rattaché » dans l'explorateur `/devis`, où un humain tranche (Rattacher / Créer le lead / Écarter). `process_pennylane_quote` créait encore un lead sans aucun appelant : à supprimer si ce n'est pas déjà fait.
- **Toute création de lead depuis l'UI passe par le filet `findPotentialDuplicates`** (fiche lead, planning, explorateur de devis), 4 axes : `client_id` (client ponté Pennylane), téléphone, email, nom+prénom. L'explorateur rattache et crée **sur la sélection** (N devis d'un même client → 1 lead). Un lead créé sans `status_id` n'est jamais promu par la RPC d'attache : toujours poser « Nouveau ».
- **Fusion de doublons = RPC `lead_merge(p_survivor_id, p_absorbed_id)`** (org_admin, bouton « Fusionner » à côté de la corbeille de la fiche lead) : additive (le survivant garde ses valeurs, hérite des champs vides, notes concaténées, compteurs additionnés, statut le plus avancé), re-parente les 16 tables FK, soft delete de l'absorbé avec instantané dans une activité `lead_merged`. **Ne jamais scripter une fusion en masse** : un client peut légitimement porter un Perdu et un Gagné.
- **Le suivi MT-LT est une VUE des projets long terme, jamais un statut** (2026-09-16). Source unique de placement = `majordhome_kanban_cards` : un lead MT-LT classé Gagné / Perdu par Pennylane sort de la liste (`LongTermTab` croise les cartes) et s'affiche dans la colonne du board (`LeadKanban` ne masque les MT-LT que des colonnes amont, `TERMINAL_COLUMN_KEYS`). Rien n'est écrit : un devis renvoyé ramène le projet. **Pas de Gagné / Perdu manuel dans le drawer MT-LT**, seul « Réactiver » reste.
- **Facturer un entretien = bouton « Facturer » de la carte Réalisé** (2026-09-21, `FacturerEntretienDialog` → `useCreateEntretienInvoice` → `pennylaneService.createInvoiceFromEntretien`, `POST /customer_invoices` via le proxy — seul POST autorisé, `finalize` / `send_by_email` bloqués tant que l'envoi client reste manuel). Spec `docs/superpowers/specs/2026-09-21-facturation-entretien-pennylane-push-design.md`. **1 ligne par équipement au prix grille, calcul IDENTIQUE au contrat signé** (`computeContractLines`, cf. § Module Contrats) ; la remise (dégressivité + commerciale + exceptionnelle) se répartit au prorata sur les lignes d'équipement, jamais sur les pièces ; TVA = `equipment_categories.default_vat_rate` (absente → 20 % + avertissement), HT = TTC / (1 + taux). Modèle PUR `src/lib/entretienInvoiceModel.js` (`node --test scripts/entretien-invoice-model.test.mjs`, dans `audit:quality`) — jamais dans le composant. Périmètre V1 : entretiens uniquement (SAV = devis PL).
- **Idempotence = mapping `pennylane_sync` type `invoice`** (`local_id` = intervention) relu AVANT tout POST : un second clic ne recrée rien. **Bouton one shot** : disparaît dès que la carte est facturée (`invoice_id` OU `invoiced_at` manuel) — avoir / facture différente = geste Pennylane, jamais une re-génération depuis la carte. Avant ce bouton, aucun chaînage facture → carte n'a jamais existé (41/50 cartes marquées à la main).
- **Lignes libres, jamais d'articles PL ; compte de vente paramétré PAR NUMÉRO** : `settings.pennylane.invoice.ledger_accounts = { by_category: { [categoryId]: '70601' }, parts: '7070' }`. Pennylane tient un compte par (numéro, taux de TVA) : `resolveLedgerAccountId(catalog, numéro, vatCode)` choisit la déclinaison du taux de la ligne, sinon `any`. Absent → défaut PL + avertissement `compte_manquant`. Les 411 (un par client) ne se paramètrent jamais. Même mécanisme à réutiliser pour les travaux. Gotcha : `getLedgerAccounts` = pagination + filtre local (`filter[number][start_with]` renvoie 400) ; `apiCall` remonte le message PL (`error.context.json()`).
- **Plan comptable de gestion = source unique des sélecteurs de compte** (2026-09-21) : `settings.pennylane.chart = [{ number, alias }]` (Settings → Socle → Plan comptable, lu par `pennylaneChart(settings)` dans `useOrgSettings.js`) ; vide → toute la classe 7 de Pennylane avec un rappel. **L'affectation est PAR MÉTIER** (l'entretien d'un poêle ≠ sa pose) : « Contrat » = catégorie d'équipement → compte (livré) ; « Devis » = un compte par article du catalogue (colonnes `supplier_products.ledger_account_pl_id` / `quote_lines.ledger_account_pl_id` posées, aucun écran ne les renseigne, à câbler avec les devis natifs) ; travaux / installation = autre module. Ne jamais mélanger les contextes.
- **`settings.pennylane.invoice.mode` = `draft` par défaut** (brouillon à finaliser/envoyer depuis PL — décision Eric 2026-09-21), `final` seulement sur son accord ; `deadline_days` (30). Éditable Settings → Socle → Facturation Pennylane (qui porte aussi le toggle `enabled`). Toujours sauver l'objet `pennylane` COMPLET (merge JSONB niveau 1).
...
- **Scénarios = paliers commerciaux, pas des variations autour de l'optimum** (révision 2026-06-18) : `buildScenarios` retourne les multiples de `OFFER_INCREMENT_KWC` (3 → 3/6/9 kWc, bornés par `min(toiture, max_power_kwc)`) + une carte « Optimisé » (`isOptimum`, étoile UI) = le `recommendedKwc` d'`optimize()`. Optimum pile sur un palier → le palier porte le flag, pas de carte doublon ; toiture < 3 kWc → carte Optimisé seule. Défaut pré-sélectionné = palier le plus proche de l'optimum, **à égalité le plus grand (ne jamais sous-dimensionner)**. Mécanique détaillée en JSDoc de `pvEngine.js`. Ne pas réintroduire de pas arbitraire (±0,5 kWc) : un palier non vendable n'a aucun sens face au client.
...
## Module Thermique (déperditions EN 12831 + dimensionnement PAC) → `docs/MODULE_THERMIQUE.md`
Outil terrain pour installateur **non-ingénieur** : déperditions pièce par pièce, dimensionnement PAC, rapport PDF client. Routes `/thermique` (wizard Contexte → Dessin/Pièces → Résultats) + `/thermique/historique` (RouteGuard `resource=thermal_study`), admin `/settings/thermique`. Règles qui mordent :
- **Moteurs PURS** (aucun import React/Supabase/alias) : `thermalEngine` (EN 12831), `heatPumpEngine` (bivalence + conso degrés-jours), `geometryEngine`, `assembleBatimentParametrique`. Toute règle de calcul se teste là — `node --test "scripts/thermique/*.test.mjs"` (331 tests) —, jamais dans un composant.
- **`buildEtudeModel` (`lib/etudeModel.js`) = source de calcul unique** écran ↔ PDF (même pattern que Solaire). `ENGINE_VERSION` versionne les règles : à incrémenter à tout changement de calcul.
- **Résultats FIGÉS (R7)** : une étude rouverte affiche `thermal_studies.results` tel qu'enregistré (bannière ambre si la version moteur diffère), **pas un recalcul** — même esprit que les contrats signés : un artefact remis au client lit les valeurs ENREGISTRÉES. `resultsPersistables(model)` définit le sous-ensemble persisté (`bilan`/`thetaE`/`pac`) ; les parois sont re-dérivables de l'`input`.
- **Rapport PDF** : `lib/rapportModel.js` (PUR) met en forme SANS jamais recalculer → un chiffre du PDF absent de l'écran est un bug de ce module. Point d'entrée **unique** des 2 boutons (étape Résultats + historique) = `telechargerRapportThermique()` de `lib/rapportExport.js` — ne pas réimplémenter la chaîne dans un 3ᵉ écran.
- **⚠️ Glyphes PDF (Helvetica/WinAnsi)** : lettres grecques (θ, Δ, Φ), flèches et `≈ ≥ ≤ −` sortent en artefact → écrire « T° extérieure de base », « majoration Utb » en toutes lettres. `° ² · × — – ’ « » € %` passent. Formatters PDF-safe obligatoires (`components/etude/pdfShared.jsx`).
