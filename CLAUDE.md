...
- Seuil pipeline **500€ HT** (2026-08-05, était 1000). Exactement 2 sources : `src/lib/constants.js` (frontend) + `supabase/functions/_shared/pipelineConstants.ts` (source unique Deno, importée par les 3 edge functions `pennylane-sync-quote-status` / `-backfill-quotes` / `-sync-cron`). Plus aucune copie inline, plus de `LEAD_THRESHOLD_HT` (refacto 2026-08-07). ⚠️ Dans `pennylane-sync-cron` le seuil ne filtre pas, il décide de **créer un lead**. ⚠️ Changer la valeur impose de redéployer les 3 edge functions (le module partagé est bundlé, pas résolu à l'exécution). Tie-break chrono = `pennylane_quote_id DESC`.
