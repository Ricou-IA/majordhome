...
// affiche un message de skip et ne fait rien : rien à convertir sans la source Open3CL.
...
if (!tvPath) {
  ...
  await convert(tvPath);
}

async function convert(tvJsPath) {
  const resolved = path.resolve(tvJsPath);
  console.log(`source : ${resolved} (clone Open3CL/engine)`);
  const { tvs } = await import(pathToFileURL(resolved).href);
