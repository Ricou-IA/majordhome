...
export const savService = {
  ...
  async getStats(orgId) {
    ...
    try {
      ...
      // Contrats en attente (status = 'pending')
      const { count: contratsEnAttente } = await supabase
        .from('majordhome_contracts')
      ...
      const stats = {
        ...
        contrats_en_attente: contratsEnAttente || 0, // Contrats en statut pending
      };
  ...
  async sendAvisRequest({ interventionId, clientId, clientFirstName, clientPhone, clientPhoneSecondary, orgId }) {
    ...
    // Collecter tous les mobiles uniques (primaire + secondaire pour Mr + Mme)
    const mobiles = [];
    const seen = new Set();
