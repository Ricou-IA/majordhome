...
// N×N, elle est (noyau) × (noyau + candidats) dans les deux sens, où le
// noyau = dépôt + arrêts déjà posés (peu nombreux, ≤ 9 en usage réel).
// Calculer candidats×candidats gaspillerait le quota sur des données jamais
// consultées — et la 1ʳᵉ version le faisait pire que ça : elle découpait TOUS
// les points (noyau+candidats mélangés) en lots bruts de 25, si bien que des
// paires noyau↔candidat tombaient ENTRE deux lots et restaient absentes en
// silence (`estime` restait `false` puisqu'aucun appel n'échouait), retombant
// sur le défaut de 60 min sans que rien ne le signale. Corrigé ici en
// distinguant explicitement noyau et candidats. Un futur appelant qui
// voudrait séquencer plusieurs candidats ENSEMBLE (les comparer entre eux)
// devra rappeler cette fonction avec ces candidats inclus dans `noyau`.
//
...
// l'org CORE (3c68193e-…), PAS l'org majordhome (7825fe43-…) qui n'existe pas
// dans `core.organizations`. `contracts`/`clients`/`pricing_equipment_types`
// portent déjà l'org core ; `appointments`/`team_members` portent l'org
// majordhome. Écrire l'org majordhome ici violerait la FK dès la première
// écriture du cache. Le paramètre est donc nommé `coreOrgId`, jamais `orgId`,
// pour ne pas reproduire l'ambiguïté vécue ailleurs dans ce projet.
// ============================================================================
...
async function fetchMatrixLot({ sourcesKeys, destinationsKeys, token, coreOrgId, paires }) {
  ...
  try {
    ...
    if (aEcrire.length) {
      ...
      if (error) logger.error('[trajets] ecriture cache impossible', error);
    }
    return false;
  } catch (err) {
    logger.error('[trajets] Mapbox indisponible — repli vol d oiseau', err);
    return true;
  }
}
...
export const trajetsService = {
   ...
   */
  async chargerMatrice({ coreOrgId, noyau, candidats }) {
    ...
    if (pairesUtiles.length > 0) {
      ...
      if (cacheError) logger.error('[trajets] lecture cache impossible', cacheError);
    ...
    if (manquantes.length > 0) {
      ...
      if (clesNoyau.length >= MAX_COORDS_MAPBOX) {
        ...
        logger.error(`[trajets] noyau de ${clesNoyau.length} points >= limite Mapbox (${MAX_COORDS_MAPBOX}) — repli vol d oiseau`);
        estime = true;
      } else if (!token) {
        logger.error('[trajets] token Mapbox absent — repli vol d oiseau');
        estime = true;
      } else {
    ...
    for (const [a, b] of pairesUtiles) {
      ...
      if (pa && pb) {
        ...
        logger.error(`[trajets] paire "${a}|${b}" sans coordonnees exploitables — impossible a estimer par vol d oiseau`);
      }
    }
